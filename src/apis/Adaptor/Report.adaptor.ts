...
export class ReportQueryAdaptor {
  ...
  static getReportListBySearch = (data?: InfiniteData<GetReportListBySearchQuery>) => ({
    pages: data?.pages ?? [],
  });
  ...
  static getRecentReportList = (data?: GetReportListBySearchQuery) => ({
    recentReportList: data?.reportList.items ?? [],
  });

  static getMostLikedReportList = (data?: GetMostLikedReportListQuery) => ({
    likedReportList: data?.items.items ?? [],
  });
}
